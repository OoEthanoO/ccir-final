...
if cupy is not None:
    try:
        X_small = cupy.array(X_train.iloc[:10].values.astype("float32"))
        y_small = cupy.array(y_train.iloc[:10].values.astype("float32"))
        dtrain_small = xgb.DMatrix(X_small, label=y_small)
        params_small = {"device": "cuda", "verbosity": 0}
        xgb.train(params_small, dtrain_small, num_boost_round=1)
        device = "cuda"
...
def rmse_cupy_scorer(y_true, y_pred):
...
neg_rmse_scorer = make_scorer(rmse_cupy_scorer, greater_is_better=False)

if device == "cuda":
    X_train = cupy.array(X_train.values.astype("float32"))
    y_train = cupy.array(y_train.values.astype("float32"))

model = xgb.XGBRegressor(objective="reg:squarederror", eval_metric="rmse", seed=42, enable_categorical=True, device=device, verbosity=2)
...
grid_search = GridSearchCV(estimator=model, param_grid=param_grid, cv=tscv, scoring=neg_rmse_scorer, verbose=3, n_jobs=1)
...
if device == "cuda":
    X_test_gpu = cupy.array(X_test.values.astype("float32"))
    preds_gpu = best_model.predict(X_test_gpu)
    preds = cupy.asnumpy(preds_gpu)
else:
...
model_path = f"models/xgb_elevation_{combined_station}_{begin_date}_{end_date}.json"
best_model.save_model(model_path)
print(f"Saved model to {model_path}")

xgb.plot_importance(best_model)
plt.title("XGBoost Feature Importance")
fi_path = f"data/xgb_elevation_feature_importance_{combined_station}_{begin_date}_{end_date}.png"
